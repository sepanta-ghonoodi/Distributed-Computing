...
def main() -> None:
    ...
    df = generate_highway(n_vehicles=args.vehicles, n_lanes=args.lanes, duration_s=args.duration, hz=10.0, seed=args.seed)
    ...
    print(f'wrote {out} | {len(df):,} rows, {df[S.VEHICLE_ID].nunique()} vehicles, {args.target_hz} Hz, mean speed {df[S.SPEED].mean():.1f} m/s')
