...
COLUMNS = [('ade', 'ADE', 8, 2), ('fde', 'FDE', 8, 2), ('ade@10s', 'ADE@10s', 9, 2), ('rmse_long', 'long', 8, 2), ('rmse_lat', 'lat', 7, 2), ('off_road', 'offroad', 9, 4), ('eta_err', 'ETAerr', 8, 2), ('interruption', 'interrupt', 10, 3), ('premature', 'wasted', 8, 2), ('detect', 'detect', 8, 3)]
...
def main() -> None:
    ...
    ap.add_argument('--variants', nargs='+', required=True, help='name=path/to/best.pt pairs, in the order they should appear')
    ap.add_argument('--split', default='test', choices=['train', 'val', 'test'])
    ap.add_argument('--snap-each', action='store_true', help='also evaluate every variant with Link Projection')
    ...
    first_cfg = config_from_dict(torch.load(variants[0][1], map_location='cpu', weights_only=False)['config'])
    cfg = first_cfg
    ...
    print(f'\ndataset  | {cfg.data.unified_path}, {args.split} split, {len(true_pos):,} windows')
    print(f'RSU      | spacing {args.rsu_spacing:.0f} m, {int(occurred.sum()):,} ({100 * float(occurred.float().mean()):.1f}%) cross a boundary')
    print(f'policy   | migration time {args.migration_time:g} s, safety margin {args.margin:g} s\n')
    ...
    def score(name: str, pred_pos: torch.Tensor, traj: dict) -> None:
        ...
        mig = migration_metrics(t_p[occurred], seen[occurred], t_true_h, args.migration_time, margin=args.margin)
        rows[name] = {**{k: traj[k] for (k, *_) in COLUMNS if k in traj}, 'off_road': off_road_rate(pred_pos, origin, theta, lane_map.to('cpu')), 'eta_err': mig['mean_eta_error_s'], 'interruption': mig['mean_interruption_s'], 'premature': mig['mean_premature_s'], 'detect': mig['handover_detect_rate']}
        lane_map.to(device)
    k = torch.arange(1, cfg.data.pred_len + 1, dtype=torch.float32).view(1, -1, 1)
    score('constant velocity', cv_delta.unsqueeze(1) * k, constant_velocity_baseline(loader, cfg.data.target_hz, device))
    for (name, path) in variants:
        ckpt = torch.load(path, map_location='cpu', weights_only=False)
        vcfg = config_from_dict(ckpt['config'])
        if vcfg.data.unified_path != cfg.data.unified_path:
            raise SystemExit(f"variant '{name}' was trained on {vcfg.data.unified_path}, but the comparison is against {cfg.data.unified_path}")
        model = build_model(NUM_FEATURES, vcfg.model).to(device)
        model.load_state_dict(ckpt['model'])
        passes = [(name, None)]
        if args.snap_each:
            passes.append((f'{name} + snap', lambda b: make_link_projection_hook(lane_map, b['origin'], b['theta'], blend=1.0, start_step=2)))
        for (label, factory) in passes:
            rep = evaluate(model, loader, cfg.train, device, pred_len=cfg.data.pred_len, target_hz=cfg.data.target_hz, hook_factory=factory, desc=label, return_predictions=True)
    ...
    print('=' * len(header))
    print(f"\nreactive migration (no prediction at all) costs {reactive_metrics(t_true_h, args.migration_time)['mean_interruption_s']:.3f} s of interruption per handover.")
    out = Path(args.out) if args.out else Path(variants[0][1]).parent / f'compare_{args.split}.json'
    with open(out, 'w') as f:
        json.dump({'split': args.split, 'rsu_spacing': args.rsu_spacing, 'migration_time': args.migration_time, 'margin': args.margin, 'rows': rows}, f, indent=2)
    print(f'wrote {out}')
