...
ROW_KEYS = ['mean_interruption_s', 'interruption_reduction_pct', 'zero_interruption_rate', 'handover_detect_rate', 'mean_eta_error_s', 'mean_premature_s']
...
def main() -> None:
    ...
    ap.add_argument('--rsu-spacing', type=float, default=300.0, help='RSU coverage diameter along the highway [m]')
    ap.add_argument('--migration-time', type=float, nargs='+', default=[1.0, 3.0, 5.0, 10.0], help='service migration durations to sweep [s]')
    ap.add_argument('--margin', type=float, default=0.0, help='safety lead time applied in the migration-duration tables [s]')
    ap.add_argument('--margins', type=float, nargs='+', default=[0.0, 1.0, 2.0, 3.0, 4.0, 6.0], help='safety lead times to sweep')
    ...
    if args.snap:
        ...
        hook_factory = lambda b: make_link_projection_hook(lane_map, b['origin'], b['theta'], blend=1.0, start_step=2)
    rep = evaluate(model, loader, cfg.train, device, pred_len=cfg.data.pred_len, target_hz=cfg.data.target_hz, hook_factory=hook_factory, desc='rollout', return_predictions=True)
    ...
    worlds = {'PAS-LGT': to_world(pred_pos, origin, theta)[..., 0], 'constant velocity': to_world(cv_pos, origin, theta)[..., 0], 'oracle': to_world(true_pos, origin, theta)[..., 0]}
    ...
    print(f'\nRSU chain | spacing {args.rsu_spacing:.0f} m over {float(origin[:, 0].min()):.0f}..{float(true_x.max()):.0f} m of highway')
    print(f'windows   | {n_total:,} total, {n_handover:,} ({100 * n_handover / n_total:.1f}%) cross an RSU boundary within {cfg.data.pred_seconds:.0f} s')
    if n_handover == 0:
        raise SystemExit('No handovers in the horizon — lower --rsu-spacing for this road length.')
    handovers = {}
    ...
    for t_m in args.migration_time:
        ...
        print(f'\n=== migration time {t_m:g} s | margin {args.margin:g} s | {n_handover:,} handovers ===')
        print(header)
        print('-' * len(header))
    ...
    with open(out_dir / f'migration_{args.split}{tag}.json', 'w') as f:
        json.dump({'rsu_spacing': args.rsu_spacing, 'n_handovers': n_handover, 'sweep': results, 'margin_sweep': margin_sweep}, f, indent=2)
    print(f"\nwrote {out_dir / f'migration_{args.split}{tag}.json'}")
    if args.plot:
        plot_sweep(results, args.migration_time, margin_sweep, t_ref, out_dir / f'migration_{args.split}{tag}.png')
