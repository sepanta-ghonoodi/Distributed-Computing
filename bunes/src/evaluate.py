"""Phase 1 test-set evaluation and qualitative plots.

    python -m src.evaluate --ckpt runs/phase1_baseline/best.pt --plot

The plot is the deliverable that motivates Phase 2: over a 30 s horizon the
baseline visibly drifts laterally off the lane it started in.
"""
...
def plot_examples(pred_pos, true_pos, out_path: Path, n: int = 6) -> None:
    ...
    for ax, p, t in zip(axes, pred_pos[:n], true_pos[:n]):
        ...
        # Equal-ish lateral scaling: a 1 m lateral error is a lane-departure
        # signal, while 1 m longitudinally is noise. Don't let autoscale hide it.
        ax.set_ylim(-6, 6)
        ax.grid(alpha=0.3)
...
def main() -> None:
    ...
    device = torch.device(
        cfg.train.device if (cfg.train.device != "cuda" or torch.cuda.is_available()) else "cpu"
    )

    datasets, _ = load_splits(cfg.data)
    loader = DataLoader(
        datasets[args.split], batch_size=cfg.train.batch_size * 2, shuffle=False
    )
